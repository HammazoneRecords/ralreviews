...
export default function Home() {
      ...
      const restaurantsWithInitialFeedback = await Promise.all(
        mockRestaurants.map(async (r) => {
          ...
          setFeedbackState(prev => ({
            ...prev,
            [r.id]: {
              customer: { items: customerResult.feedback, lastVisible: customerResult.lastVisible, hasMore: customerResult.feedback.length > 0 && customerResult.lastVisible !== null },
              employee: { items: employeeResult.feedback, lastVisible: employeeResult.lastVisible, hasMore: employeeResult.feedback.length > 0 && employeeResult.lastVisible !== null }
            }
          }));
  ...
  const handleFawud = (restaurantId: string, feedbackId: string) => {
    ...
    const updater = (prevRestaurant: Restaurant) => {
      const updatedFeedback = prevRestaurant.feedback.map(f => {
        if (f.id === feedbackId) {
          return { ...f, fawuds: (f.fawuds || 0) + 1 };
        }
        return f;
      });
      return { ...prevRestaurant, feedback: updatedFeedback };
    };
  ...
  const handleAddFeedback = async (restaurantId: string, feedbackData: Omit<Feedback, 'id' | 'fawuds' | 'author' | 'improvementAreas' | 'finalComment' | 'qualityScore'>): Promise<Feedback> => {
    ...
    const feedbackType = newFeedbackWithScore.type; // 'customer' or 'employee'
    setFeedbackState(prev => {
  ...
  const handleLoadMoreFeedback = useCallback(async (restaurantId: string, feedbackType: 'customer' | 'employee') => {
    ...
    setFeedbackState(prev => ({
        ...
        [restaurantId]: {
            ...
            [feedbackType]: {
                ...
                hasMore: newFeedback.length > 0 && lastVisible !== null,
            }
        }
    }));
  
    // We also need to update the main `restaurants` array to ensure consistency everywhere
    setRestaurants(prevRestaurants => 
      prevRestaurants.map(r => {
        if (r.id === restaurantId) {
          const otherFeedbackType = feedbackType === 'customer' ? 'employee' : 'customer';
          const otherItems = feedbackState[restaurantId]?.[otherFeedbackType]?.items || [];
          return { ...r, feedback: [...combinedFeedback, ...otherItems] };
        }
        return r;
      })
    );
    ...
    if (selectedRestaurant?.id === restaurantId) {
      setSelectedRestaurant(prev => {
        ...
        const otherFeedbackType = feedbackType === 'customer' ? 'employee' : 'customer';
        const otherItems = feedbackState[restaurantId]?.[otherFeedbackType]?.items || [];
        return { ...prev, feedback: [...combinedFeedback, ...otherItems] };
      });
    }
  }, [feedbackState, selectedRestaurant]);
